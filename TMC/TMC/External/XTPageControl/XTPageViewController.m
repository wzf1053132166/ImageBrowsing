...
- (void)viewDidLayoutSubviews {
    ...
    self.tabBar.frame = CGRectMake(_frame_x, self.height, self.width, self.tabBarHeight);
    
    self.underlineView.frame = CGRectMake(0, self.height + self.tabBarHeight, self.view.bounds.size.width, 1.0);
    
    self.pageScrollView.frame = CGRectMake(0, self.height + self.tabBarHeight + 1.0, self.view.bounds.size.width, self.view.bounds.size.height - self.tabBarHeight - 1.0 - [self.topLayoutGuide length]);
...
- (void)willChanged:(NSInteger)preIndex nextIndex:(NSInteger)nextIndex button:(UIButton *)button{
    
    if (self.delegate && [self.delegate conformsToProtocol:@protocol(XTPageViewControllerDelegate)]) {
        [self.delegate indexFlag:nextIndex];
    }
    ...
    if (self.forceToShowControllerWhenFirstTime) {
        ...
        if (!self.disableScroll) {
            ...
            if (_isScroll == 1) {
                [self.pageScrollView setContentOffset:CGPointMake(nextIndex * self.pageScrollView.bounds.size.width, 0) animated:NO];
            }else{
...
- (void)didChanged:(NSInteger)preIndex nextIndex:(NSInteger)nextIndex button:(UIButton *)button{
